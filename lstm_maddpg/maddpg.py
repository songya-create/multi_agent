import torch
import os
from lstm_maddpg.lstm_actor_critic  import Actor, Critic
from torch.utils.tensorboard import SummaryWriter
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
writer1 = SummaryWriter("logs/add_scalar1")
writer2 = SummaryWriter("logs/add_scalar2")
class MADDPG:
    def __init__(self, args, agent_id):  # 因为不同的agent的obs、act维度可能不一样，所以神经网络不同,需要agent_id来区分
        ...
        #学习率下降设置
        # self.actor_scheduler = torch.optim.lr_scheduler.LambdaLR(self.actor_optim, lr_lambda=lambda1)
        # self.critic_scheduler = torch.optim.lr_scheduler.LambdaLR(self.critic_optim, lr_lambda = lambda1)
        ...
        if os.path.exists(self.model_path + '/2_actor_params.pkl'):
            ...
            print('Agent {} successfully loaded actor_network: {}'.format(self.agent_id,
                                                                          self.model_path + '/2_actor_params.pkl'))
            print('Agent {} successfully loaded critic_network: {}'.format(self.agent_id,
                                                                           self.model_path + '/2_critic_params.pkl'))
    ...
    def train(self, transitions, other_agents):
        ...
        u[self.agent_id] = self.actor_network(o[self.agent_id])###o[agent_id,batch,seq*state]
        ...
        if self.agent_id == 0 and self.train_step % 100 == 0 :

          #  print('critic_loss is {}, actor_loss is {}'.format(critic_loss, actor_loss))

            writer1.add_scalar('agent0_critic_loss', critic_loss,self.train_step)
            #writer1.close()
            writer2.add_scalar('agent0_actor_loss', actor_loss,self.train_step)
           # writer2.close()
